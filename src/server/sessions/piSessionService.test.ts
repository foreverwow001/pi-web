...
function testModel(): NonNullable<PiAgentSession["model"]> {
  const model = ModelRegistry.inMemory(AuthStorage.inMemory()).find("anthropic", "claude-sonnet-4-5-20250929");
...
describe("PiSessionService", () => {
  ...
  it("refreshes auth state and dedupes warnings when logout removes the current model's credentials", async () => {
    ...
    const warningCount = () => hub.sessionEvents.filter(({ event }) => event.type === "command.output" && event.level === "error" && event.message.includes("anthropic/claude-sonnet-4-5-20250929")).length;
