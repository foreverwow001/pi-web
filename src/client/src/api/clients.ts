import type { DeleteWorkspaceFileResponse, FileSuggestion, IvyhouseFooterMode, MoveWorkspaceFileOptions, PiWebConfigValues, PromptAttachment, RunTerminalCommandInput, SessionBulkMutationRef, SessionCleanupRequest, SessionRef, TerminalCommandRun, TerminalCommandRunFilter, WriteWorkspaceFileOptions } from "../../../shared/apiTypes";
...
export const sessionsApi = {
  ...
  prompt: (session: SessionLookup, text: string, attachmentsOrStreaming?: unknown[] | string, streamingOrMachine?: string, maybeMachineId = "local") => {
    const hasAttachments = Array.isArray(attachmentsOrStreaming);
    const attachments = hasAttachments ? attachmentsOrStreaming : [];
    const streamingBehavior = hasAttachments
      ? normalizeStreamingBehavior(streamingOrMachine)
      : normalizeStreamingBehavior(attachmentsOrStreaming);
    const machineId = hasAttachments
      ? maybeMachineId
      : typeof attachmentsOrStreaming === "string" && streamingBehavior === undefined
        ? attachmentsOrStreaming
        : typeof streamingOrMachine === "string" && normalizeStreamingBehavior(streamingOrMachine) === undefined
          ? streamingOrMachine
          : maybeMachineId;
