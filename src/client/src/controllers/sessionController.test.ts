...
function imageAttachmentPayload(): PromptAttachmentPayload {
  return {
    id: "attachment-shot",
    kind: "image",
    filename: "shot.png",
...
describe("SessionController", () => {
  ...
  it("refreshes the selected transcript tail when idle status reports newer messages", async () => {
    ...
    controller.applyGlobalEvent({ type: "status.update", status: { ...status(oldSession.id), messageCount: 3 } });
    runPendingAnimationFrames();
    await vi.waitFor(() => { expect(state.messagePageTotal).toBe(3); });
  ...
  it("toggles the per-session sending state around an inline attachment send and forwards attachments", async () => {
    ...
    let promptArgs: { attachments?: PromptAttachmentPayload[] } | undefined;
    let state: AppState = { ...initialAppState(), selectedWorkspace: workspace, selectedSession: oldSession, sessions: [oldSession] };
    const attachments: PromptAttachmentPayload[] = [imageAttachmentPayload()];
    const api: typeof defaultApi = {
      ...defaultApi,
      prompt: (_session, _text, sentAttachments) => new Promise<{ accepted: true }>((resolve) => {
        promptArgs = sentAttachments === attachments ? { attachments } : {};
    ...
    const send = controller.send("look", attachments);
  ...
  it("queues prompt sends for a pending session start and flushes them after resolution", async () => {
    ...
    const api: typeof defaultApi = {
      ...
      messages: () => Promise.resolve(emptyPage),
      status: (session) => Promise.resolve(status(sessionLookupId(session))),
      prompt: (session, text, _attachments, behavior) => {
        const normalizedBehavior = behavior === "steer" || behavior === "followUp" ? behavior : undefined;
        promptCalls.push({ sessionId: sessionLookupId(session), text, ...(normalizedBehavior === undefined ? {} : { behavior: normalizedBehavior }) });
  ...
  it("queues slash commands, shell input, and attachments for a pending session start", async () => {
    ...
    const startRequest = deferred<SessionInfo>();
    const calls: string[] = [];
    const promptCalls: { text: string; attachments?: PromptAttachmentPayload[] }[] = [];
    const attachments: PromptAttachmentPayload[] = [imageAttachmentPayload()];
    let state: AppState = { ...initialAppState(), selectedWorkspace: workspace, sessions: [] };
    const api: typeof defaultApi = {
      ...defaultApi,
      ...
      prompt: (session, text, sentAttachments) => {
        calls.push(`prompt:${sessionLookupId(session)}:${text}`);
        promptCalls.push({ text, ...(sentAttachments === attachments ? { attachments } : {}) });
        return Promise.resolve({ accepted: true });
      },
    };
    const controller = new SessionController(
    ...
    await controller.send("look", attachments);
    ...
    expect(promptCalls).toEqual([{ text: "look", attachments }]);
    expect(state.clientQueuedSessionMessages[started.id]).toBeUndefined();
  });
